...
pub fn get_notes() -> NoteFile {
    let note_file = std::fs::read_to_string("notes.toml").unwrap();
    let notes: NoteFile = toml::from_str(&note_file).expect("Invalid notes file please fix any issues in it or delete it to generate a new one.");
    notes
}
